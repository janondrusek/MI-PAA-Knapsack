...
public class SimulatedAnnealingProblemSolver {
     ...
     * @see ure.phd.simulatedannealing.interfaces.AnnealingScheduler#init()
     * 0. init annealingSchedular
     ...
     * 1. OR until reaching the limit for number of tries @SimulatedAnnealingProblem.isTotalNumberOfStatesReached()
     ...
     *       1.1.2 find the difference between the next state and the current state<br/>
     *       1.1.3 if the delta is less then 0 then go to next state<br/>
     *       1.1.3 OR if not use Boltzman function in order to decide main purpose of accepting an increment state is not to get stuck in a local minimum
     ...
     *   @see AnnealingScheduler#coolDown(double)
     *   1.2 cool it down<br/>
     *
     */
